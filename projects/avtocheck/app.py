...
# Момент, когда плечи начали различаться ЦЕНОЙ (раунд 5). До него они
# различались вёрсткой экрана при одной цене, и те показы к текущему тесту
# отношения не имеют — иначе под одними буквами сравнивались бы разные вещи.
VISUAL_TEST_SINCE = "2026-08-09T11:20:00"
...
def _an_panels(ctx: dict) -> list[dict]:
    ...
    # --- Цена отчёта: A/B (раунд 5, с 09.08.2026) -----------------------------
    ...
    try:
        # Пол теста: до VISUAL_TEST_SINCE плечи различались не ценой.
        ...
        if seen_total:
            ...
            panels.append({
                ...
                "table": {"head": ["Плечо", "Увидели цену", "Нажали оплату", "Оплатили", "₽ с показа"],
                          "rows": rows},
                "hint": (
                    "Плечи различаются ЦЕНОЙ полного отчёта: "
                    + " · ".join(f"<b>{k}</b> — {v} ₽" for k, v in CS_VARIANTS.items())
                    + ". Экран у всех один (тест вёрстки закрыт 09.08). "
                    "Знаменатель — сессии, которые ДОШЛИ до экрана цены, а не все назначенные. "
                    "Решать ТОЛЬКО по столбцу <b>₽ с показа</b>: у дешёвого плеча конверсия "
                    "выше почти всегда, а денег оно может приносить меньше. "
