...
/* Переключатель периода: сегментами в одной оболочке, чтобы читался как ОДИН
   орган управления, а не четыре независимые кнопки рядом. */
.psegs{display:inline-flex;background:var(--card);border:1px solid var(--line-2);
  border-radius:9px;padding:2px;gap:2px}
.psegs .pseg{font-size:12.5px;line-height:1;padding:6px 10px;border-radius:7px;
  text-decoration:none;color:var(--ink-2);white-space:nowrap;transition:.12s}
.psegs .pseg:hover{background:var(--hover)}
.psegs .pseg.on{background:var(--ink);color:var(--card);font-weight:640}
@media(max-width:720px){.psegs{order:9;width:100%;justify-content:space-between}}
...
def _wiring_block(d: dict) -> str:
    ...
    # Половинная ширина, а не full: иначе карточка занимает целую строку и уезжает
    # под «Прочие сигналы», хотя обе короткие и просятся рядом.
    return ('<div class="card"><h2>' + _ic("plug") + 'Состояние подключения'
...
def _period_switch(d: dict) -> str:
    """Переключатель периода в шапке: сегодня / вчера / с начала теста / всё время.

    Раньше окно было одно и задавалось снаружи, поэтому на вопрос «сколько
    сегодня?» приходилось считать в уме. Ссылки, а не JS: работает без скриптов
    и переживает перезагрузку — период виден в адресе."""
    cur = d.get("period") or "window"
    tok = _q(str(d.get("campaign_token", "")))
    src = _q(str(d.get("src_selected", "all") or "all"))
    camp = _q(str(d.get("campaign_selected", "all") or "all"))
    items = "".join(
        f'<a class="pseg{" on" if k == cur else ""}" '
        f'href="?token={tok}&src={src}&campaign={camp}&period={k}">{_esc(label)}</a>'
        for k, label in (("today", "Сегодня"), ("yesterday", "Вчера"),
                         ("window", "С начала теста"), ("all", "Всё время")))
    return f'<div class="psegs">{items}</div>'


def render_dashboard(cfg, d: dict[str, Any]) -> str:
    since = (d.get("since") or "").replace("T", " ")[:16]
    until = (d.get("until") or "").replace("T", " ")[:16]
    period = d.get("period") or "window"
    # Подпись именно тем окном, что реально применено: если написать «сегодня», а
    # считать с начала теста, отчёт будет врать молча.
    since_txt = ({"today": "сегодня", "yesterday": "вчера", "all": "за всё время"}.get(period)
                 or (f'с {since}' if since else 'за всё время'))
    if until:
        since_txt += f' (до {until})'
    ...
    body = (
        ...
        _lift_block(d) +
        _signals_block(d) + _wiring_block(d))
    return (
