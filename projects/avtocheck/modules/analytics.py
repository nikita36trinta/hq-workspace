...
class AnalyticsConfig:
    ...
    admin_token: str
    title: str = "Аналитика"
    # Какой период открывается по умолчанию: today / yesterday / window / all.
    # «window» тянет от точки since_provider — это правильно, пока продукт не менялся.
    # Как только меняются цены или экраны, старые данные в одном окне с новыми
    # усредняют разное, поэтому проект вправе открывать дашборд на «сегодня».
    default_period: str = "window"
    # Пол данных: всё, что раньше этой метки, дашборд не показывает НИ В ОДНОМ
    # периоде, включая «Всё время». Нужен, когда продукт сменился настолько, что
    # старые цифры не сравнимы с новыми и усреднять их — врать себе.
    data_floor: str | None = None
...
# Периоды дашборда. Раньше окно было одно и задавалось извне (начало A/B-теста),
# поэтому вопрос «а сколько сегодня?» приходилось считать в голове.
PERIODS = (("today", "Сегодня"), ("yesterday", "Вчера"),
           ("window", "С начала теста"), ("all", "Всё время"))


def _window(cfg: AnalyticsConfig, period: str) -> tuple[str | None, str | None]:
    """(since, until) в ISO-UTC для выбранного периода. until не включается.

    Границы суток берём по МСК, а не по UTC: владелец смотрит «сегодня» своими
    глазами, и в 02:00 МСК отчёт «за сегодня» не должен показывать вчерашний день.
    """
    from datetime import datetime, timedelta, timezone
    msk = timezone(timedelta(hours=3))
    now = datetime.now(msk)
    start = now.replace(hour=0, minute=0, second=0, microsecond=0)
    iso = lambda d: d.astimezone(timezone.utc).isoformat(timespec="seconds").replace("+00:00", "")
    floor = cfg.data_floor
    def _clamp(since_iso):
        """Не пускаем окно ниже пола данных."""
        if not floor:
            return since_iso
        return floor if (since_iso is None or since_iso < floor) else since_iso

    if period == "today":
        return _clamp(iso(start)), None
    if period == "yesterday":
        y0, y1 = iso(start - timedelta(days=1)), iso(start)
        # Период целиком ниже пола — показываем пусто, а не подтягиваем к полу:
        # иначе «Вчера» рисовало бы сегодняшние числа под чужой подписью.
        if floor and y1 <= floor:
            return floor, floor
        return _clamp(y0), y1
    if period == "all":
        return floor, None            # «всё время» = всё, что после пола
    return _clamp(_since(cfg)), None  # «window»
...
def build_dashboard(cfg: AnalyticsConfig, ctx: dict | None = None) -> dict[str, Any]:
    ...
    ctx = ctx or {}
    period = str(ctx.get("period") or cfg.default_period or "window")
    if period not in {k for k, _ in PERIODS}:
        period = "window"
    since, until = _window(cfg, period)
    roles = cfg.roles
    ...
    if matrix_on:  # пред-проход: кампания каждой сессии (кука cs_camp липкая, но visit до неё без метки)
        for line in lines:
            try:
                ...
                continue
            _ts = r.get("ts") or ""
            if (since and _ts < since) or (until and _ts >= until):
                continue
    ...
    if cfg.payments_provider:
        try:
            ...
            payments = []
    def _in_window(x) -> bool:
        t = x.get("ts") or x.get("paid_at") or ""
        return (not since or t >= since) and (not until or t < until)
    paid = [x for x in payments if x.get("status") == "succeeded" and _in_window(x)]
    ...
    return {
        ...
        "src_selected": ctx.get("src", "all") or "all",
...
def _net_by_variant(cfg, paid) -> list | None:
    ...
    if not cfg.price_variants or not paid:
        return None
    # Цена у всех плеч одна — таблица «по варианту цены» сравнивала бы одно и то же
    # с самим собой и только занимала бы экран. Вернётся, когда цены снова разойдутся.
    if len(set(cfg.price_variants.values())) <= 1:
        return None
    cogs = {}
...
def selfcheck(cfg: AnalyticsConfig, since: str | None = None,
    ...
    Окно обязательно: без него карточка показывала счётчики за всю историю рядом с
    дашбордом за сегодня — 14 833 визита против 874 в шапке. Числа с одной страницы
    не должны спорить друг с другом.
    """
    v = cfg.validate()
    p = _events_path(cfg)
...
def make_router(cfg: AnalyticsConfig) -> APIRouter:
    ...
    def dashboard(token: str = "", src: str = "all", campaign: str = "all",
                  period: str = "") -> Response:
        # Умолчание берём из конфига проекта, а не зашиваем «window» здесь: иначе
        # default_period в конфиге ничего не решает — параметр приходит уже заполненным.
        period = period or cfg.default_period or "window"
        if not cfg.admin_token or token != cfg.admin_token:
            raise HTTPException(status_code=403, detail="forbidden")
        ctx = {"src": src, "token": token, "campaign": campaign, "period": period}
        data = build_dashboard(cfg, ctx)
        # То же окно, что у всего дашборда: иначе карточка спорит с шапкой.
        data["selfcheck"] = selfcheck(cfg, data.get("since"), data.get("until"))
    ...
    def dashboard_json(token: str = "", src: str = "all", campaign: str = "all",
                       period: str = "window") -> JSONResponse:
        if not cfg.admin_token or token != cfg.admin_token:
            raise HTTPException(status_code=403, detail="forbidden")
        return JSONResponse(build_dashboard(
            cfg, {"src": src, "token": token, "campaign": campaign, "period": period}))

    @router.get("/admin/selfcheck")
    def selfcheck_route(token: str = "") -> JSONResponse:
        if not cfg.admin_token or token != cfg.admin_token:
            raise HTTPException(status_code=403, detail="forbidden")
        # Отдельный эндпоинт — диагностика подключения, там окно ни к чему:
        # событие могло прийти на прошлой неделе, и это всё равно значит «подключено».
