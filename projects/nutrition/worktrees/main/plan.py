...
def compute(quiz: dict) -> dict:
    ...
    cal = bmr * af
    goal = quiz.get("goal")
    # ЖЕЛАЕМЫЙ ВЕС решает наравне с целью. Раньше он спрашивался в квизе и не читался
    # НИГДЕ: человек с ростом 200 и весом 90 указывал желаемые те же 90, выбирал
    # «снизить вес» — и получал план на дефиците 20%, то есть на снижение веса,
    # которого не просил. Ответ на прямо заданный вопрос нельзя выбрасывать: либо
    # спрашиваем и считаемся с ним, либо не спрашиваем.
    #
    # Цель и число расходятся → верим ЧИСЛУ: оно конкретнее. Дефицит держим только
    # когда человек действительно назвал вес меньше нынешнего, прибавку — когда
    # больше. Разницу меньше килограмма считаем «тем же весом»: это шум ввода.
    try:
        target = float(b.get("target") or 0)
    except Exception:  # noqa: BLE001
        target = 0.0
    if target > 0:
        if goal == "lose" and target >= w - 1:
            goal = "keep"
        elif goal == "gain" and target <= w + 1:
            goal = "keep"
    if goal == "lose":
        cal *= 0.80
    elif goal == "gain":
...
def _shopping(sh: list, days: list | None = None) -> str:
    ...
    def _cat_for(k: tuple, roots: list) -> str:
        ...
        if cat:
            return cat
        # Главное слово — не первое. В русском первым обычно идёт прилагательное:
        # у «красного лука» roots[0] = «красн», а он занят «чечевицей красной» —
        # и лук уезжал в «Бакалею». Берём САМЫЙ ДЛИННЫЙ корень: у составного
        # названия это почти всегда существительное («лук», «хлопья», «сок»).
        for head in sorted(roots, key=len, reverse=True):
...
def _shopping_flat(sh: list) -> str:
    ...
    имеет права."""
    if not sh:
        # Пустой экран вместо раздела — худший исход: человек платил в том числе
        # за список покупок, открывает вкладку и видит НИЧЕГО, даже без слова
        # «пусто». Так бывает у банк-заготовки: ни ингредиентов у блюд, ни
        # недельного списка. Говорим правду и обещаем ровно то, что делает крон.
        return ("<section class='sec'><h2>Список покупок</h2>"
                "<div class='prefcard'><p class='phint'>Список ещё собирается вместе с "
                "рецептами — он появится здесь, как только меню будет готово. Обычно это "
                "занимает до часа; мы пришлём письмо.</p>"
                "<p class='phint'>Если списка не будет и через несколько часов — напиши "
                "на <a href='mailto:support@mynutriplan.ru'>support@mynutriplan.ru</a>, "
                "разберёмся.</p></div></section>")
    cats, total = "", 0
