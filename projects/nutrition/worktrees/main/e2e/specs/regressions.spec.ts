...
test.describe("regressions · пример дня на пейволле", () => {
	...
	test("в строке дня крупным идёт название блюда, а не приём пищи", async ({ page }) => {
		...
		// Раньше здесь стояло «по умолчанию обед — рыба». Это держалось на дефекте:
		// сервер показывал завтрак/обед/ужин ВСЕМ, не читая ответ про число приёмов,
		// и выбравший «3 + перекусы» видел обещание пяти приёмов над днём из трёх.
		// Помощник жмёт первый вариант на каждом шаге, то есть отвечает «2 приёма», —
		// обеда в таком дне нет по ответу человека, а не по недосмотру.
		const meals = await page.evaluate(
			() => (JSON.parse(localStorage.getItem("np_quiz_v2") ?? "{}").answers ?? {}).meals ?? "3",
		);
		const want: Record<string, number> = { "2": 2, "3": 3, "3s": 5, if: 2 };
		expect(await rows.count(), `ответ «${meals}» → столько приёмов в примере дня`).toBe(
			want[meals] ?? 3,
		);
		// Подпись приёма набрана капителью через CSS, innerText отдаёт «УЖИН».
		expect(shown.toLowerCase(), "последний приём дня — ужин").toContain("ужин");
	});
