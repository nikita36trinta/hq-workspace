...
def _manage_link(base: str, token: str) -> str:
    """Ссылка на управление подпиской в письме с планом. ОТКЛЮЧЕНА 2026-08-03 по
    решению владельца: управление живёт только в личном кабинете.

    Оферта (п. 6) переписана в тот же день — раньше она обещала эту ссылку в
    каждом письме с планом, и оставить обещание при снятой ссылке значило бы
    держать в договоре неправду.

    Функция не удалена: вызовы во всех письмах с планом на местах, вернуть —
    снять флаг. Иначе пришлось бы заново вспоминать, что разовому покупателю
    ссылку давать нельзя (подписки у него нет, и она врала бы о её наличии)."""
    if not PLAN_MAIL_MANAGE_LINK:
        return ""
...
# Письма о деньгах по подписке (активация, предупреждение за 3 дня, факт
# списания) сняты по решению владельца 2026-08-03: письмо с датой будущего
# списания почтовые клиенты превращают в напоминание накануне, а оно читается
# как приглашение отменить.
#
# Без подтверждения платежа человек при этом НЕ остаётся: в запрос на продление
# мы кладём receipt с его почтой, и ЮKassa шлёт фискальный чек по 54-ФЗ на
# каждое списание. Это требование закона и отключить его нельзя.
#
# Вернуть письмо — поставить True. Дедупликация и все вызовы на местах.
SUB_CHARGE_MAILS = False

# Ссылка «управление подпиской» в письмах с планом. Снята там же и тогда же:
# управление живёт только в личном кабинете. Оферта переписана под это.
PLAN_MAIL_MANAGE_LINK = False
