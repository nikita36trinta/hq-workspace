...
def _prompt(quiz: dict, p: dict, avoid: list[str] | None = None) -> tuple[str, str]:
    ...
    user = (
        ...
        # Без этой строки модель делит дневную норму НАЦЕЛО между приёмами: у
        # человека с двумя приёмами выходило 1130 и 1130 — и так все семь дней,
        # четырнадцать одинаковых чисел подряд. Выглядит как заглушка, потому
        # что это она и есть: никто так не ест.
        "- Калорийность приёмов РАЗНАЯ и правдоподобная: сытнее там, где основной\n"
        "  приём, легче — где перекус или поздний ужин. Не дели норму поровну и не\n"
        "  повторяй одно и то же число день за днём; расхождение между приёмами\n"
        "  одного дня — обычно 15–40%.\n"
...
def _plan_problems(days, cal: int, quiz: dict | None = None,
    ...
    for idx, d in enumerate(days):
        ...
        # Норма, поделённая нацело: у двух приёмов это 1130 и 1130, и так семь
        # дней. Формально план в коридоре, а на экране — четырнадцать одинаковых
        # чисел, по которым сразу видно, что считал не человек. Претензия мягкая:
        # ретрай с пояснением, но не уход в банк-заготовку без рецептов.
...
def _fix_note(hard: list[str], soft: list[str], quiz: dict | None = None) -> str:
    ...
    if any(i.startswith("even:") for i in soft):
        parts.append("ты поделил дневную норму поровну между приёмами — так не едят; "
                     "сделай приёмы разной калорийности (расхождение 15–40%) и не "
                     "повторяй одно и то же число во всех днях")
