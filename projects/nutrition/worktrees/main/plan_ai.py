...
# Доли суточной калорийности по приёмам.
#
# Не «на глаз»: 3 приёма — это завтрак 25–30%, обед 35–40%, ужин 20–25% плюс
# перекусы (МР 2.3.1.2432-08, те же цифры лежат в основе любого школьного и
# санаторного меню). Перекус в норме 5–10%, поэтому в раскладке с перекусами
# основные приёмы ужимаются, а не добавляются сверху.
MEAL_LAYOUT = {
    ...
    # Интервальное окно 8 часов: завтрака в нём нет по определению. Первый приём
    # в окне делаем основным, второй легче — так его проще съесть и он не мешает сну.
    "if": [("Обед", "lunch", 0.55), ("Ужин", "dinner", 0.45)],
}

# Выше этого порога один приём становится нереалистичным: столько за один раз
# просто не съедают, и человек либо не доедает, либо бросает план. Практический
# потолок обычной порции — около 800–900 ккал; берём 850.
MAX_MEAL_KCAL = 850


def meal_layout(quiz: dict | None, cal: int = 0) -> list[tuple[str, str, float]]:
    """Приёмы дня под ответ квиза; неизвестный ответ — обычные три приёма.

    При большой суточной норме раскладка РАСТЯГИВАЕТСЯ. Причина не в теории:
    человек с нормой 2820 ккал и режимом «интервальное окно» получал два приёма
    по 1400 ккал — столько за раз не ест никто, и белок в 85 г за присест
    усваивается хуже, чем те же 170 г, разнесённые по дню. Добавляем перекусы,
    пока средний приём не станет обычной порцией; за 6 приёмов не выходим —
    дальше это уже не план питания, а расписание.
    """
    lay = list(MEAL_LAYOUT.get(str((quiz or {}).get("meals") or ""), MEAL_LAYOUT["3"]))
    # Добавляем РОВНО ОДИН перекус и только если самый тяжёлый приём выходит за
    # обычную порцию. Смотрим на максимум, а не на среднее: по среднему четыре
    # приёма считались нормой, пока обед весил 1260 ккал.
    #
    # Один, а не «сколько понадобится»: перекус даёт 15% дня, и чтобы дотянуть
    # тяжёлый приём до 850 ккал при норме под 3000, их пришлось бы добавить
    # четыре. Интервальное голодание с четырьмя перекусами — это уже не режим, а
    # его отрицание, и человек такого не заказывал. При высокой норме крупные
    # приёмы неизбежны, и честнее оставить их крупными, чем ломать выбранный
    # формат питания.
    if cal and not any(k == "snack" for _, k, _ in lay) \
            and max(s for _, _, s in lay) * cal > MAX_MEAL_KCAL:
        # 10%, как в норме для перекуса, а не 15. На 15% при суточной норме 2800
        # перекус получал 420 ккал, а модель ставила туда яблоко или варёное яйцо
        # и подписывала им эти 420 — число расходилось с едой. Перекус на 280 ккал
        # уже честно закрывается горстью орехов или творогом с ягодами.
        share = 0.10
        lay = [(l, k, s * (1 - share)) for l, k, s in lay]
        lay.insert(len(lay) - 1, ("Перекус", "snack", share))
    return lay
...
def _prompt(quiz: dict, p: dict, avoid: list[str] | None = None) -> tuple[str, str]:
    ...
    layout = meal_layout(quiz, p["cal"])
    # Калории КАЖДОГО приёма называем числом. Раньше модель получала только норму
    # дня и число приёмов — и делила норму нацело: у двух приёмов выходило 1130 и
    # 1130, и так все семь дней. При этом раскладка с долями у нас была, просто
    # до промпта не доезжала. Профессиональный план распределяет энергию по
    # приёмам заранее, а не оставляет это на усмотрение того, кто пишет меню.
    per_meal = ", ".join(f"{lbl} ≈{int(round(p['cal'] * sh / 10) * 10)} ккал"
                         for lbl, _, sh in layout)
    meals = (f"{meals} — РОВНО {len(layout)} приёма(ов) в КАЖДОМ дне, "
             f"в порядке: {', '.join(lbl for lbl, _, _ in layout)}.\n"
             f"Калорийность приёмов: {per_meal} (±15%)")
    ...
    user = (
        ...
        # Разнообразие — то, за что платят: план на неделю, где семь ужинов из
        # курицы и рыбы, человек составит и сам. Требование конкретное, иначе
        # модель тянется к десятку «безопасных» блюд.
        "- РАЗНООБРАЗИЕ: ни одно блюдо не повторяется за всю неделю. Источники\n"
        "  белка чередуй (рыба, птица, мясо, яйца, творог, бобовые, тофу) — ни один\n"
        "  не чаще чем в трети приёмов. Гарниры и овощи тоже меняй.\n"
        # Модель подгоняла ккал под заданную цифру, не глядя на еду: «Яблоко
        # свежее — 400 ккал», «Яйцо варёное — 400 ккал». Число обязано следовать
        # за составом и порцией, а не наоборот.
        "- ккал и БЖУ обязаны соответствовать составу и ПОРЦИИ из ingredients:\n"
        "  яблоко не бывает на 400 ккал. Нужен калорийный приём — бери сытное\n"
        "  блюдо и укажи вес в ингредиентах, а не подписывай большое число мелкой еде.\n"
        # Иначе в перекус на 280 ккал попадали огурец с сельдереем: в них столько
        # нет и в двойной порции, а уменьшить число модель не может — оно задано.
        "  Для перекуса свыше 200 ккал выбирай плотную еду (орехи, творог, сыр,\n"
        "  бутерброд, смузи), а не сырые овощи и не один фрукт — в них таких ккал нет.\n"
...
def _plan_problems(days, cal: int, quiz: dict | None = None,
    ...
    for idx, d in enumerate(days):
        ...
        # Приём не должен быть неподъёмным. Проверяем по РАСКЛАДКЕ, а не «числа
        # должны отличаться»: раскладка и есть ответ на вопрос, сколько где.
        # Отдельно ловим одинаковую калорийность всех приёмов — это норма,
        # поделённая нацело, и на экране она читается как заглушка.
        ks = [int(m.get("kcal") or 0) for m in meals]
        if len(ks) > 1 and len(set(ks)) == 1 and ks[0] > 0:
            soft.append(f"even:{idx}")
        # Сравниваем с ЦЕЛЬЮ ЭТОГО приёма, а не с общим потолком: при норме 2800
        # в интервальном окне обед и должен быть большим, и придираться к нему
        # бессмысленно — ретраи не сделают день короче. Ловим другое: модель
        # ушла от заданного распределения больше чем в полтора раза.
        if cal:
            targets = [cal * sh for _, _, sh in meal_layout(quiz, cal)]
            if len(targets) == len(ks) and any(k > t * 1.5 for k, t in zip(ks, targets) if t):
                soft.append(f"huge:{idx}")
    ...
    # РАЗНООБРАЗИЕ по всей неделе. Проверка «то же блюдо в том же приёме два дня
    # подряд» пропускала ровно то, на что жалуются: рыба–курица–рыба–курица через
    # день формально ей не противоречит. За неделю человек платит именно за то,
    # чтобы не думать И не есть одно и то же; повтор — это невыполненное обещание.
    names = [(m.get("name") or "").strip().lower()
             for d in days for m in ((d or {}).get("meals") or []) if (m.get("name") or "").strip()]
    if names and len(set(names)) < len(names) * 0.85:
        soft.append("variety")
    return hard, soft
...
def _fix_note(hard: list[str], soft: list[str], quiz: dict | None = None) -> str:
    ...
    if any(i.startswith("even:") for i in soft):
        parts.append("ты поделил дневную норму поровну между приёмами — держись "
                     "калорийности приёмов, заданной выше")
    if any(i.startswith("huge:") for i in soft):
        parts.append("какой-то приём получился в полтора раза тяжелее заданного — "
                     "держись калорийности приёмов из задания")
    if "variety" in soft:
        parts.append("слишком много повторов: ни одно блюдо не должно встречаться "
                     "в неделе дважды, источники белка чередуй")
