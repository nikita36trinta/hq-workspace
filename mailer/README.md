...
- `MAILIN_TG_TOKEN` / `MAILIN_TG_CHAT` / `MAILIN_TG_PROXY` — уведомления о входящей почте в телеграм.
  **Ставить сразу вместе с приёмником.** Иначе ящик — это папка на диске, куда никто не заходит:
  на ЧистойСделке три обращения клиентов (в том числе требование вернуть деньги) пролежали
  непрочитанными по 2–4 суток, а узнали мы о них случайно. Прокси обязателен: с РФ-серверов
  `api.telegram.org` недоступен напрямую. `MAILIN_TG_SKIP` (деф. `noreply`) глушит уведомления
  о собственных служебных письмах — без этого каждый тест рассылки будит оператора,
  и он перестаёт смотреть на уведомления вообще.
